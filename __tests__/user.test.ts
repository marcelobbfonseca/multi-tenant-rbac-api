...
describe('GET /api/v1/users', () => {
   ...
   let accessToken: string;
    ...
    beforeEach(async () => {
      ...
      accessToken = signJWTAccessToken('email', 1, 1, 'admin');
    ...
    test('should create a new user and return 201 CREATED',async () => {
      ...
      const req = await request(app).post('/api/v1/users').set('Authorization', `bearer ${accessToken}`).send(params);
    ...
    test('should list users and return 200 OK', async () => {
        ...
        const req = await request(app).get('/api/v1/users').set('Authorization', `bearer ${accessToken}`);
    ...
    test('should find by id and return 200 OK', async () => {
        ...
        const req = await request(app).get(`/api/v1/users/${user.id}`).set('Authorization', `bearer ${accessToken}`);
